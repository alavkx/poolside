...
async function loadReleaseNotesConfig(configPath: string): Promise<ReleaseNotesConfig> {
  try {
    ...
    config.releaseConfig = {
      ...config.releaseConfig,
      includeTableOfContents: config.releaseConfig.includeTableOfContents ?? true,
...
program
  ...
  .action(
    async (epicId: string, options: { agent: string; claimant?: string; verbose?: boolean }) => {
      try {
        validateConfig('epic');
...
program
  ...
  .option('--verbose', 'Enable verbose logging for debugging')
  .action(async (projectKey: string, options: { limit: string; verbose?: boolean }) => {
    try {
      validateConfig('epic');
  ...
  .action(
    async (options: {
      ...
      try {
        ...
        console.log(
          chalk.gray(`   • Target month: ${config.releaseConfig.month || getCurrentMonth()}`)
        );
        console.log(
          chalk.gray(`   • Output file: ${config.releaseConfig.outputFile || 'release-notes.md'}`)
        );
...
program
  ...
  .option('-o, --output <file>', 'Output config file path', 'release-config.json')
  .action(async (options: { output: string }) => {
    try {
      ...
      try {
        ...
        await fs.copyFile(examplePath, options.output);
        console.log(
          chalk.green(`✅ Release notes configuration template created: ${options.output}`)
        );
        console.log(
          chalk.yellow(
        ...
        console.log(chalk.green(`✅ Basic release notes configuration created: ${options.output}`));
        console.log(
          chalk.yellow('📝 Update the configuration with your actual repository details.')
        );
      }
    } catch (error: any) {
...
program
  ...
  .action(() => {
    ...
    console.log(
      chalk.gray('• Release Notes: Requires OPENAI_API_KEY, GITHUB_TOKEN (JIRA optional)')
    );
  });
