...
export class JiraPATManager {
  ...
  isPAT(password) {
    ...
    return password && (
      password.length > 20 || 
      password.match(/^[A-Za-z0-9]{20,}$/) ||
      password.startsWith('ATATT') // Atlassian Cloud PAT prefix
    );
  }
  ...
  async createPAT(tokenName = 'Release Notes Generator', expirationDays = 365) {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        ...
        throw new Error('Permission denied. You may not have permission to create PATs, or PATs may be disabled.');
      } else if (error.response?.status === 404) {
        throw new Error('PAT endpoint not found. Your JIRA instance may not support PATs or may be an older version.');
      } else {
  ...
  async setupPATWorkflow() {
    console.log(chalk.cyan('\n🔐 JIRA Personal Access Token Setup'));
    console.log(chalk.gray('For better security, we recommend using Personal Access Tokens instead of passwords.'));
    ...
    if (!credentialsWork) {
      ...
      console.log(chalk.yellow('Please check your JIRA_HOST, JIRA_USERNAME, and JIRA_PASSWORD settings.'));
      return null;
    }
    ...
    try {
      ...
      console.log(chalk.gray('Expires:'), patResult.expiresAt ? new Date(patResult.expiresAt).toLocaleDateString() : 'Never');
